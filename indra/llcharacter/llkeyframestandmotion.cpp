...
BOOL LLKeyframeStandMotion::onUpdate(F32 time, U8* joint_mask)
{
	...
	{
		LLVector4a dirLeft4;
		dirLeft4.loadua((F32*)mAnkleLeftJoint.getWorldMatrix().getFwdRow4().mV);
		LLVector4a dirRight4;
		dirRight4.loadua((F32*)mAnkleRightJoint.getWorldMatrix().getFwdRow4().mV);
