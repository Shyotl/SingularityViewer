...
BOOL LLVOVolume::genBBoxes(BOOL force_global)
{
	...
	for (S32 i = 0; i < getVolume()->getNumVolumeFaces(); i++)
	{
		...
		LLMatrix4a mat;
		mat.loadu((F32*)mRelativeXform.mMatrix);
...
void LLVOVolume::generateSilhouette(LLSelectNode* nodep, const LLVector3& view_point)
{
	...
	if (volume)
	{
		...
		LLMatrix4a tmata;
		tmata.loadu((F32*)trans_mat.mMatrix);
		LLMatrix4a imata;
		imata.loadu((F32*)mRelativeXformInvTrans.mMatrix);
		volume->generateSilhouetteVertices(nodep->mSilhouetteVertices, nodep->mSilhouetteNormals, view_vector, tmata, imata, nodep->getTESelectMask());
