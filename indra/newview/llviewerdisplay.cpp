	...
	if (!gDisconnected)
	{
		...
		{ 
			...
			if (!for_snapshot || LLPipeline::sRenderDeferred)
			{
				...
				const glh::matrix4f& proj = glh_get_current_projection();
				const glh::matrix4f& mod = glh_get_current_modelview();
...
void render_hud_attachments()
{
	...
	const glh::matrix4f saved_proj = glh_get_current_projection();
	const glh::matrix4f saved_mod = glh_get_current_modelview();
...
bool get_hud_matrices(const LLRect& screen_region, LLMatrix4a &proja, LLMatrix4a &modela)
{
	if (isAgentAvatarValid() && gAgentAvatarp->hasHUDAttachment())
	{
		...
		glh::matrix4f mat(modela.getF32ptr());
		...
		tmp_model *= mat;
		glh::matrix4f model = tmp_model;

		proja.loadu(proj.m);
		modela.loadu(model.m);
		return TRUE;
	}
	else
	{
...
void render_ui(F32 zoom_factor, int subfield, bool tiling)
{
	...
	const glh::matrix4f saved_view = glh_get_current_modelview();
